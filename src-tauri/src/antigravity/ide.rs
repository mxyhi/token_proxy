...
pub(crate) async fn switch_ide_account(
    app: &AppHandle,
    store: &AntigravityAccountStore,
    account_id: &str,
    path_override: Option<PathBuf>,
    ...
    ide_status(app, Some(&config)).await
}

pub(crate) async fn ide_status(
    app: &AppHandle,
    cached: Option<&AntigravityIdeConfig>,
) -> Result<AntigravityIdeStatus, String> {
    let config = match cached {
        Some(config) => config.clone(),
        None => resolve_ide_config(app).await?,
    };
...
async fn resolve_ide_config(app: &AppHandle) -> Result<AntigravityIdeConfig, String> {
    let config = crate::proxy::config::read_config(app.clone()).await?.config;
