...
pub(crate) struct AntigravityAccountStore {
    dir: PathBuf,
...
impl AntigravityAccountStore {
    pub(crate) fn new(app: &AppHandle, app_proxy: AppProxyState) -> Result<Self, String> {
        let dir = config_dir_path(app)?.join(ANTIGRAVITY_AUTH_DIR_NAME);
        Ok(Self {
            dir,
    ...
    pub(crate) async fn list_accounts(&self) -> Result<Vec<AntigravityAccountSummary>, String> {
